import torch
import util.data_processer as processer
import data.clue as clue
import data.word2vec as w2v
from util.data_processer import DataConfig, DataSetFactory, DataProcess
...
if __name__ == '__main__':
    ...
    data_process = DataProcess(config.data)
    if en_prep:
        clue.conv2json()
        processer.conv2json()
        w2v.conv2pkl()
    ...
    data_factory = DataSetFactory(config.fitting)
    ...
    for fold_index in range(0, config.fitting.fold_num if config.fitting.en_cross else 1):
        ...
        if config.fitting.en_cross and fold_index == config.fitting.fold_num - 1 or False:
            processer.combine_dev_result(ROOT_RESULT, config.fitting.fold_num,
                                       ROOT_RESULT + 'dev_result_all.json')
            processer.analyze_dev_data(ROOT_RESULT + 'dev_result_all.json',
                                     processer.ROOT_LOCAL_DATA + 'seg_train_data.json',
                                     verbose=True)
    ...
    if en_conv:
        if config.fitting.en_cross:
            ...
            processer.mix_clue_result(ROOT_RESULT + 'test_result_vote.json',
                                    ROOT_DATA + 'clue/train_dev_test_data.json',
                                    ROOT_RESULT + 'test_result_vote_mix_clue.json')
            processer.convert2csv(ROOT_RESULT + 'test_result_vote_mix_clue.json',
                                ROOT_RESULT + 'predict_vote.csv')
        else:
            processer.mix_clue_result(ROOT_RESULT + 'test_result.json',
                                    ROOT_DATA + 'clue/train_dev_test_data.json',
                                    ROOT_RESULT + 'test_result_mix_clue.json')
            processer.convert2csv(ROOT_RESULT + 'test_result_mix_clue.json',
